...
def home():
    ...
    return render_template('index.html', download_folder=download_folder, video_quality=video_quality, video_resolution=video_resolution)
...
def video_settings():
    ...
    if custom_resolution == "yes":
        ...
        video_resolution = 'bv[height<=' + video_resolution + ']+ba/best[height<=' + video_resolution + ']'
    ...
    print("URL: " + video_url)
    start_download(video_url, video_data)
    return redirect(url_for("home"))

#@app.route('/download', methods=["GET", "POST"])
def start_download(video_url, video_data):
    global download_thread
    #video_url = request.form.get("yt-link")
    if not download_thread:
        download_thread = True
        thread = threading.Thread(target=download, daemon=True, args=(video_url, video_data,))
        thread.start()
    return

def download(video_url, video_data):
    global download_thread
    if video_url:
